...
var driver = neo4j.driver("bolt://localhost", neo4j.auth.basic("neo4j", "semantics"));
...
app.post('/location/add', function(req, res){
    ...
    session
        .run("CREATE(n:Place {city:{cityParam}, country:{countryParam}}) RETURN n", {cityParam: city, countryParam:country})
        .then(function(result){
            res.redirect('/');
            session.close();
        })
...
app.post('/person/born/add', function(req, res){
    ...
    session
        .run("MATCH(a:Person {name:{nameParam}}),(b:Place {city:{cityParam}, country:{countryParam}}) MERGE(a)-[r:BIRTH {year:{yearParam}}]->(b) RETURN a,b", {nameParam: name, cityParam:city, countryParam:country, yearParam:year})
        .then(function(result){
            if(id && id != null){
                res.redirect('/person/'+id);
            } else{
...
app.get('/person/:id', function(req, res){
    ...
    session
        ...
        .then(function(result){
            ...
            session
                .run("OPTIONAL MATCH (a:Person)-[r:BIRTH]-(b:Place) WHERE id(a)=toInt({idParam}) RETURN b.city as city, b.country as country", {idParam:id})
                .then(function(result2){
